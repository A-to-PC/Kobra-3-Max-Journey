...
> **TL;DR** — The enclosure's exterior got its final coat, the corrected `.2 PLA` profile went on a real print, and the retraction line's cable clips got hot-glued down to stop them rattling. Separately, rather than keep guessing at the real filament hub's internals, the actual reference STLs got pulled into Blender and measured directly — confirming it's a two-part Cap-feeds-Body design, and turning up ready-made 4-port and 5-port versions of the exact same hub plus a proper multi-tube organiser clip, all from the same designer, meaning no from-scratch rebuild is even needed. Also worked out a real bulk-filament buying and storage plan, sized against the dryer drawers this build has been putting together all along. Later in the day, a Full Cal re-run (now that the printer's fully enclosed) came with a visibly calmer shake test, and the corrected profile's real result landed: the Day 25 wall-to-bottom-shell gap is down to roughly 5% of its previous size — call it fixed. Kobra LAN Monitor also picked up wheel-zoom on the camera feed (v1.0.7), tested live against this build's actual printer before shipping.
...
### Full Cal, re-run enclosed — and the gap fix confirmed for real

With the enclosure now fully sealed and sitting on its finished pallet base, a Full Cal got re-run — the printer had only ever auto-calibrated itself once, on first power-on, standing in the open. Worth noting even before the numbers came back: the input-shaping shake test itself was visibly calmer than it used to be. More mass and rigidity under the printer from the pallet base, plus the enclosure now damping the whole structure, means less resonance getting transmitted in the first place — a real, physical reason to expect a tighter calibration, not just a hopeful one.

The corrected `.2 PLA` profile from earlier today then got a proper test print. Result: the Day 25 wall-to-bottom-shell gap — the one traced back to the `bottom_shell_layers`/`bottom_shell_thickness` conflict — is down to roughly 5% of what it was. Effectively fixed. The only defect on the plate was pure human error (forgot to enable support on one object, that one part needs a reprint), nothing traceable to the printer, the profile, or the calibration.

One change from today hasn't been tested yet, and it's worth being precise about which is which: `top_bottom_infill_wall_overlap` also got raised to roughly 25-30% today, to match Jason's own already-boosted `infill_wall_overlap` — a separate fix to a separate setting, made after the gap-confirming print, not part of what that print actually tested. A same-PLA, different-colour 5-hour print went on straight after to test it properly.

### Kobra LAN Monitor: wheel-zoom on the camera feed (v1.0.7)

A small but genuinely useful addition to the dashboard used to watch this build's printer: scroll the wheel over the live camera feed and it zooms in toward wherever the cursor is, via a CSS transform on the image itself rather than resizing the camera card — so the layout never jumps around mid-zoom. Works on both the onboard and network camera sources. Tested live against the real printer on a throwaway second instance before shipping, confirmed working, then built, reinstalled, and pushed as v1.0.7.

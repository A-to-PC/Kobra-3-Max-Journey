...
With that fixed, a real upload finally completed: Kobra Slicer's own UI showed "COMPLETED" for the first time, and the printer's touchscreen showed genuine "Downloading files, Please Wait" with a real progress bar.

### Upload and print turned out to be two separate signals — again

The printer then sat on "Downloading files, Please Wait" indefinitely, never completing or erroring. Watching Kobra LAN Monitor's dashboard, a fresh Ex/Im log export off the printer, and another packet capture (which found genuine MQTT traffic on port 9883 right after the HTTP upload response — real, but TLS-encrypted, so not directly readable) all failed to pin down a cause directly.

The resolution came from an unrelated event: the printer got power-cycled for other reasons, and the model that had appeared stuck "downloading" the night before was found already sitting on the machine, not yet printed. The upload had genuinely completed and been valid the whole time — only the visible progress/completion state on-screen was ever stuck, not the file itself.

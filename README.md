...
>
> **A note on the shape of this log, added Day 16.** Reading back through the earlier chapters, they lean heavily on describing the technical outcome — what got found, what got fixed — without always showing the actual back-and-forth behind it: whose hunch started a given test, what got physically checked by hand rather than reasoned from a screen, which mistake got caught by which of us. Read in isolation, that makes it look like less happened on the human side than actually did. That's a real gap in how this was written, not in what actually happened — every real finding in here started from a real question, a real guess worth testing, or a real physical check, same as the note above already says. From here on, the day-by-day entries try to show more of that side directly — real photos, real reasoning, real decisions — not just the polished result.
...
## Chapter 8: RFID Filament Tags, Making the ACE Pro Actually Automatic

> **TL;DR** — The ACE Pro can read a filament tag per slot and auto-fill material and colour instead of typing it in by hand every load, but only if the tag holds the right structured data. A generic NFC read/write app can't produce that — worse, it can *permanently lock a tag* before you even realise it wrote the wrong thing. A purpose-built filament-tag Android app got it right on the first real attempt, confirmed both by an immediate correct read on the printer and by a manual-override test that proved it's genuinely automatic, not a one-off fluke.
...
### The first attempt, and the real cost of guessing blind
...
### Switching to a purpose-built app
